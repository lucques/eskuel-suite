...
test('constructs and exports the minimal SQLite play-through game', async ({ page }) => {
    ...
    await newGameModal.locator('#new-game-name').fill('minimal-playthrough');
    ...
    await expect(title.getByLabel('Name:')).toHaveValue('minimal-playthrough');
    ...
    await visibleSceneTextArea(editSceneModal).fill('Welcome to the minimal game.');
    await editSceneModal.getByRole('button', { name: 'Save' }).click();
    await waitForEditorCommit(page);
    await expect(scenesPanel.getByText('Welcome to the minimal game.', { exact: true })).toBeVisible();
    ...
    await replaceEditorContents(
        ...
        'UPDATE items SET collected = 1 WHERE name = \'key\'',
    );
    await replaceEditorContents(
        page,
    ...
    await visibleSceneTextArea(finalTextModal).fill('You finished the minimal game.');
    await finalTextModal.getByRole('button', { name: 'Add' }).click();
    await waitForEditorCommit(page);
    await expect(scenesPanel.getByText('You finished the minimal game.', { exact: true })).toBeVisible();
    await expect(scenesPanel.locator('[data-scene-emphasis="prominent"]')).toHaveCount(4);

    await title.getByLabel('Name:').fill('Minimal Playthrough E2E');
    await title.getByLabel('Name:').press('Tab');
    await waitForEditorCommit(page);
    await page.getByText('Additional information', { exact: true }).click();
    await metadata.getByLabel('Teaser').fill('A minimal game for the browser play-through test.');
    ...
    const databasePanelHeight = await databasePanel.evaluate(element => element.getBoundingClientRect().height);
    const schemaPanelHeight = await schemaPanel.evaluate(element => element.getBoundingClientRect().height);
    expect(schemaPanelHeight).toBeGreaterThanOrEqual(databasePanelHeight - 1);
    ...
    expect(savedFile.suggestedFilename()).toBe('minimal_playthrough.xml');
    ...
    await invalidDatabaseChooser.setFiles({
        name: 'invalid.sql',
        mimeType: 'text/plain',
        buffer: Buffer.from('This is not SQL.'),
    });
